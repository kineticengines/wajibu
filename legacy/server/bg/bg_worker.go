...
var listOfFuncs []func()

func init() {
	//fetch all functions the add to a list
	listOfFuncs = append(listOfFuncs, cacheProcessor)
}

func StartBG() {
	assemble(listOfFuncs)
}

func assemble(fn []func()) {
	workBook := make(chan func(), 100)
	for _, v := range listOfFuncs {
		workBook <- v
	}
	go doWork(workBook)
}

func doWork(job chan func()) {
	for j := range job {
		go j()
	}
