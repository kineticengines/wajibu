...
func GetSentimentFields() *[]string {
	res, err := RDB.Cmd("SMEMBERS", SENTIMENT).List()
	report.ErrLogger(err)
	return &res
}

func CacheQuery(item string) *bool {
	var rAll bool
	err := RDB.Cmd("SADD", CACHEDSEARCH, item).Err
	if err != nil {
		rAll = false
	}
	rAll = true
...
func GetCachedQueryItems() *[]string {
	res, err := RDB.Cmd("SMEMBERS", CACHEDSEARCH).List()
	report.ErrLogger(err)
	return &res
}
